...
export default function Request({ pageType }: { pageType: RequestPageType }) {
    ...
    return (
        <div className="Cardlike Cardlike--IsReset">
            <title>{computePageTitle(pageType)}</title>

            <h2 className="Cardlike__Title">{computeCardTitle(pageType)}</h2>

            {hasAttempted && isSuccessful ? successNotice : resetRequestForm}
        </div>
    );
}
