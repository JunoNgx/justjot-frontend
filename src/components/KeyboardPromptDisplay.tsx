...
export default function KeyboardPromptDisplay() {
    const { isMainInputFocused, selectedIndex, selectedItem } =
    ...
    const primaryAction =
        selectedItem && computeItemPrimaryAction(selectedItem);
    const primaryActionStr =
        primaryAction && computeItemActionString(primaryAction);
    const canConvertToTodo = selectedItem && canConvertItemToTodo(selectedItem);

    const expandedContent = (
        <div className="KeyboardPrompt__List" aria-hidden={!isExpanded}>
            <KeyboardPromptItem
                prompt={["mod", "F"]}
                desc="Main Input"
...
const KeyboardPromptItem = ({
    ...
    desc,
    shouldDisplay = true,
}: KeyboardPromptItemOptions) => {
    if (!shouldDisplay) return "";

    return (
        <div className="KeyboardPrompt__Item">
...
const SingleKeyboardPrompt = ({ prompt }: { prompt: KeyboardPrompt }) => {
    // Process and unify param into single type
    const promptList: Hotkey[] =
        prompt[0] instanceof Array ? (prompt as Hotkey[]) : [prompt as Hotkey];

    return (
        <div className="KeyboardPrompt__Prompt">
