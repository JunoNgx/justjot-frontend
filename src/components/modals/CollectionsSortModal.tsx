...
export default function CollectionsSortModal() {
    const { initCollections, fetchCollections } =
    ...
    const handleDragEnd = ({ destination, source }: DropResult) => {
        ...
        if (destination!.index === 0) {
            ...
            const newSortOrderValue =
                firstCollection.sortOrder - COLLECTION_SORT_ORDER_MAG;
        ...
        if (destination!.index === state.length - 1) {
            ...
            const newSortOrderValue =
                lastCollection.sortOrder + COLLECTION_SORT_ORDER_MAG;
            processUpdateCollectionSortOrder(
                movedCollection,
                destination!.index,
                newSortOrderValue
        ...
        const newSortOrderValue =
            (prevCollection.sortOrder + nextCollection.sortOrder) / 2;
        processUpdateCollectionSortOrder(
            movedCollection,
            destination!.index,
            newSortOrderValue
    ...
    const processUpdateCollectionSortOrder = async (
        movedCollection: ItemCollection,
        newIndex: number,
        newSortOrderValue: number
    ) => {
        itemsHandlers.setItem(newIndex, {
            ...movedCollection,
    ...
    const draggableItemList = state.map((collection, index) => (
        <Draggable
            key={collection.id}
            index={index}
        ...
        >
            {(provided, _snapshot) => {
                ...
                return (
                    <div
                        className="Modal__CollectionItem"
                        {...provided.draggableProps}
                        {...provided.dragHandleProps}
                        ref={provided.innerRef}
                    >
                        <p className="Modal__CollectionName">
                            {collection.name}
                        </p>
                    </div>
                );
    ...
    const draggableArea = (
        <DragDropContext onDragEnd={handleDragEnd}>
            <Droppable droppableId="collection-list" direction="vertical">
                {(provided) => (
    ...
    return (
        <div className="Modal--IsCollectionSort">
            <div className="Modal__SortCollectionList">
                <div className="Modal__LeftSide">{draggableArea}</div>
            </div>
        </div>
    );
}
