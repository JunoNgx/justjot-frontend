...
export default function ItemComponentIcon({ item }: { item: Item }) {
    ...
    if (item.isPending) return <IconHourglassLow {...itemIconProps} />;

    if (isValidHexColourCode(lastSevenChars) && item.type !== ItemType.TODO)
        return (
            <div
                className="Item__IconColour"
