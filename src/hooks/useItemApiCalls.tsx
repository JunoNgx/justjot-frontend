...
export default function useItemApiCalls() {
    ...
    const updateItemTitle = async ({
        ...
        await pbClient
            .collection(DbTable.ITEMS)
            .update(itemId, { title }, { requestKey: "item-update-title" })
            .then((record: Item) => {
                successfulCallback?.(record);
            })
            .catch((err) => {
    ...
    const updateItemContent = async ({
        ...
        await pbClient
            .collection(DbTable.ITEMS)
            .update(itemId, { content }, { requestKey: "item-update-content" })
            .then((_record: Item) => {
                successfulCallback?.();
            })
            .catch((err) => {
    ...
    const toggleItemShouldCopyOnClick = async ({
        ...
        await pbClient
            .collection(DbTable.ITEMS)
            .update(item.id, { shouldCopyOnClick }, { requestKey: null })
            .then((record) => {
                successfulCallback?.(record);
            })
            .catch((err) => {
    ...
    const toggleItemIsTodoDone = async ({
        ...
        await pbClient
            .collection(DbTable.ITEMS)
            .update(item.id, { isTodoDone: isTodoDone }, { requestKey: null })
            .then((record) => {
                successfulCallback?.(record);
            })
            .catch((err) => {
    ...
    const trashItem = async ({
        ...
        setLoadingState?.(true);
        await pbClient
            .send(`items/trash/${user!.id}/${item!.id}`, {
                method: "PATCH",
                authorization: pbClient.authStore.token,
            })
            .then((record) => {
    ...
    const untrashItem = async ({
        ...
        setLoadingState?.(true);
        await pbClient
            .send(`items/untrash/${user!.id}/${item!.id}`, {
                method: "PATCH",
                authorization: pbClient.authStore.token,
            })
            .then((record) => {
