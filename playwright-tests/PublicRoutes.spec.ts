...
test.describe("Navigating public routes", () => {
    test.describe("Using pointer", () => {
        ...
        test("Landing page", async ({ page }) => {
            await expect(
                page.getByRole("heading", { name: "JustJot" })
            ).toBeVisible();
            await expect(
                page.getByRole("link", { name: "Try now" })
            ).toBeVisible();
            await expect(
                page.getByRole("link", { name: "Login", exact: true })
            ).toBeVisible();
            await expect(
                page.getByRole("banner").getByRole("link", { name: "Register" })
            ).toBeVisible();
        });
        ...
        test("Login page", async ({ page }) => {
            ...
            await expect(
                page.getByRole("link", { name: "Request reset" })
            ).toBeVisible();
            await expect(
                page.getByRole("link", { name: "Request verification" })
            ).toBeVisible();
        });
        ...
        test("Register page", async ({ page }) => {
            ...
            await expect(
                page1.getByRole("heading", { name: "Terms and Conditions" })
            ).toBeVisible();
        });
        ...
        test("Reset password page", async ({ page }) => {
            ...
            await expect(
                page.getByRole("heading", { name: "Request password change" })
            ).toBeVisible();
        });
        ...
        test("Verify email page", async ({ page }) => {
            await page
                ...
                .click();
            await page
                .getByRole("link", { name: "Request verification" })
                .click();
            await expect(
        ...
        test("Demo login page", async ({ page }) => {
            ...
            await expect(
                page.getByText("Try using the test account")
            ).toBeVisible();
        });
    });
